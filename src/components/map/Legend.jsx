...
const Legend = ({ editEnabled, x, y, position }) => {
  const { map, legendRef } = useContext(GlobalMapContext);
  const [title, setTitle] = useState(
    map.currentMapProprietaryJSONOriginal.legend.title
  );
  // pull legendPairs from global state
  const [legendPairs, setLegendPairs] = useState(
    map.currentMapProprietaryJSONOriginal.legend.keyValueLabels
  );
  ...
  return (
    <div className="legend" style={{ left: x, bottom: y, position: position }}>
      <div className="legend-header">
        {(isEditing && editEnabled) ? (
          ...
          <div className="legend-title" onClick={() => setIsEditing(true)}>
            {title}
          </div>
        )}
      </div>
      <div className="legend-body">
        {map.currentMapProprietaryJSON &&
          (map.currentMapProprietaryJSON.templateType === "bin" ||
            map.currentMapProprietaryJSON.templateType === "cadastral") && (
                  ...
                  <tr key={key}>
                    <td>
                      <input
                        ...
                        defaultValue={key}
                        onChange={(e) =>
                          handleValueChange(e.target.value, value)
                        }
                      ></input>
                    </td>
                    {editEnabled ? (
                      <>
                        <td>
                          <input
                            type="text"
                            className="legend-bin-color-label"
                            defaultValue={value}
                            onChange={(e) =>
        ...
        {map.currentMapProprietaryJSON &&
          map.currentMapProprietaryJSON.templateType === "landmark" && (
            <div className="legend-landmark">
              <table class="legend-landmark-table">
                {markers.map((marker) => (
                  <tr>
                    <td>
                      <span className="material-icons">{marker}</span>
                    </td>
                    <td>
                      <input
                        type="text"
                        ...
                        onClick={(e) =>
                          handleValueChange(marker, e.target.value)
                        }
                      />
                    </td>
                  </tr>
